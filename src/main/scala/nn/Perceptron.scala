...
trait Network extends LazyLogging {
  ...
  def trainIteration(input: DenseMatrix[Double], output: DenseMatrix[Double]): Network
...
case class Perceptron(
  ...
  def activate(inputs: DenseMatrix[Double]): DenseMatrix[Double] =
    (DenseMatrix.horzcat(inputs, bias(inputs.rows)) * weights).map(activationFunction)
  ...
  def trainIteration(
    ...
    val newWeights = this.weights - eta * (DenseMatrix.horzcat(inputs, bias(inputs.rows)).t * errors)
...
object Perceptron extends LazyLogging {
  ...
  /**
   * Creates a perceptron, adds the a bias entry in the weights matrix
   * 
   */
  ...
  def train(p: Perceptron, inputs: DenseMatrix[Double], targets: DenseMatrix[Double], epochs: Int): Perceptron = {

    logger.debug("Starting training")
    logger.debug("Initial weights:\n"+p.weights.toString(10, 10))

    @annotation.tailrec
    def loop(p: Perceptron, inputs: DenseMatrix[Double], targets: DenseMatrix[Double], loops: Int): Perceptron = {
      logger.debug(s"Training epoch $loops")
      if (loops <= epochs) loop(p.trainIteration(inputs, targets), inputs, targets, loops+1)
      else p
    }
